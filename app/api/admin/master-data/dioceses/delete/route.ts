...
export async function POST(req: NextRequest) {
  ...
  if (!id || !isUuid(id)) {
    return NextResponse.json(
      { error: "ValidationError", message: "id keuskupan wajib diisi dan harus UUID valid." },
      { status: 400 },
    );
  }
  ...
  const cleanupSpecs: CleanupSpec[] = [
    { table: "profiles", column: "diocese_id", mode: "set_null", label: "Profil User" },
    { table: "posts", column: "diocese_id", mode: "set_null", label: "Postingan (posts)" },
    { table: "user_posts", column: "diocese_id", mode: "set_null", label: "Postingan User" },
  ];
  ...
  const referenceSpecs = [
    ...cleanupSpecs,
    { table: "churches", column: "diocese_id", mode: "delete", label: "Paroki" } satisfies CleanupSpec,
  ];

  const { error: deleteError } = await adminClient.from("dioceses").delete().eq("id", id);
  if (deleteError) {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "Tidak bisa menghapus keuskupan karena data ini masih dipakai oleh tabel relasi lain.",
          references,
        },
        { status: 409 },
      );
      ...
      return NextResponse.json(
        {
          error: "PermissionDenied",
          message:
            "Service role belum punya izin hapus di tabel dioceses. Jalankan SQL GRANT terlebih dahulu.",
        },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      { error: "DatabaseError", message: message || "Gagal menghapus keuskupan." },
      { status: 500 },
    );
  }
  ...
  const res = NextResponse.json({
    success: true,
    message: "Keuskupan berhasil dihapus.",
    data: existing,
  });
